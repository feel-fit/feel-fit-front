<template>
  <div class="container-fluid py-5 my-3 px-0">
    <div class="row header-interes align-items-center">
      <div class="col-12">
        <img :src="recipe.photo" alt class="header-image" />
      </div>
      <div class="row margen-receta">
          ...
          <div class="container-fluid my-5">
            <div class="d-flex">
              ...
              <div>
                ...
                <p
                  class="cite mt-5"
                >{{recipe.author}} | {{articulo.created_at|moment("from", "now")}}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="col-12">
          <img :src="articulo.photo" alt class="text-image" />
        </div>
        <div>
          <p>{{articulo.text_footer}}</p>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    api
      ...
      .then(response => {
        next(vm => {
          vm.articulo = response.data.data;
        });
      });
  }
};
